...
import { isAdminUser, getUserWithPreferences, getAllBrands, getUserBrands, updateUser } from '@/lib/db/client-repos'
...
export default function FavoriteBrands() {
    ...
    const checkAuth = async () => {
      ...
      if (userData.has_favorite_brands) {
        ...
        if (userBrandsData && userBrandsData.length > 0) {
          setSelectedBrands(userBrandsData)
        }
      }

      // Fetch available brands from master table
      const { data: brandsData, error: brandsError } = await getAllBrands()
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!data.ok) {
        const errorMessage = data.error?.message || 'Failed to save brands'
